...
print(f"\n{'=' * 60}")
print(f"开始训练多输出随机森林模型...")
print(f"{'=' * 60}")

# 3. 创建随机森林管道（RandomForestRegressor原生支持多输出，不需要额外参数）
pipeline = Pipeline([
...
save_data(best_estimator, 'output/models/multi_output_rf_model.joblib')
print(f"\n模型已保存到：output/models/multi_output_rf_model.joblib")

# 5. 评估训练集和测试集性能（原代码的测试集评估部分）
print(f"\n{'=' * 60}")
print(f"测试集性能评估")
print(f"{'=' * 60}")
...
save_data(evaluation_results, 'output/models/rf_multioutput_evaluation.joblib')
...
for target in y_train.columns:
    ...
    print(f"{target}:")
    print(f"  训练集 R²: {train_r2:.4f}, 测试集 R²: {test_r2:.4f}, 差异: {test_r2 - train_r2:.4f}")
    print(f"  训练集 RMSE: {train_rmse:.4f}, 测试集 RMSE: {test_rmse:.4f}, 比率: {test_rmse / train_rmse:.4f}")
    print()