...
bayes_search_spaces = {
    'Eff.COD': {
        ...
        'model__min_samples_split': Integer(7, 20),
    ...
    'Eff.NO3--N': {
        ...
        'model__min_samples_leaf': Integer(3, 8),
        'model__max_features': Categorical(['sqrt', 'log2', None]),
    },
    'Eff.NH4+-N': {
        'model__max_depth': Integer(3, 7),
        'model__min_samples_split': Integer(3, 10),
        'model__min_samples_leaf': Integer(3, 5),
        'model__max_features': Categorical(['sqrt', 'log2', None]),
    }
}
...
for target in y_train.columns:
    pipeline = Pipeline([
        ...
        ('model', DecisionTreeRegressor(random_state=42))  # 使用决策树
    ])

    # 使用针对每个目标的贝叶斯搜索空间
    search_space = bayes_search_spaces.get(target, {
        'model__max_depth': Integer(3, 8),  # 默认搜索空间
        'model__min_samples_split': Integer(2, 10),
    ...
    save_data(bayes_search.best_estimator_, f'output/models/dt_model_{target}.joblib')  # 文件名前缀dt_
    print(f"模型已保存到：output/models/dt_model_{target}.joblib")